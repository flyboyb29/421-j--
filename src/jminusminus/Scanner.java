		...
		while (moreWhiteSpace) {
			...
			if (ch == '/') {
				...
				case '*': //multi-line comments
					String eml = ""; // end of multi-line
					char last = ' '; // last charactor
					// sets the next two chars to check for the combo "*/"
					nextCh();
					eml = eml + ch;
					nextCh();
					eml = eml + ch;
					// sets to keep track of the last char to be in current combo
					// to be used as the first part of next check combo
					last = ch;
					while (!eml.equals("*/") && ch != EOFCH) {
						// update to new combo to check for "*/"
						nextCh();
						eml = "";
						eml = eml + last + ch;
						last = ch;
...
//				return getNextToken();
		...
		case '~':
			nextCh();
			return new TokenInfo(BCOMPLIMENT, line);	
		case '%':
			nextCh();
			if (ch == '=') {
				nextCh();
		...
		case '>':
			...
			if (ch == '>') {			//bitshift
				...
				if (ch == '>') {
					...
					if (ch == '=') {
						nextCh();
						return new TokenInfo(RSHIFTZ_ASSIGN, line);
					} else {
						return new TokenInfo(RSHIFTZ, line);
					}
				} else if (ch == '=') {
					nextCh();
...
//				return getNextToken();
		...
		case '"':
			...
			//Float and Double literals partially handled here
		case '.':
			buffer = new StringBuffer();
            buffer.append(ch);
            nextCh();
            if (isDigit(ch)) { 
                buffer.append(ch);
                nextCh();
                // Boolean is used to ensure there is at minimum 1 following digit for validity 
                boolean followingDigits = true;
                while (isDigit(ch)) {
                    buffer.append(ch);
                    nextCh();
                }
        ...
        case '0':
            ...
            if (ch == 'x' || ch == 'X') { 
                ...
                if (followingDigits) {
                    ...
                    reportScannerError("The Hex Literal is incorrect");
                }
            ...
            //BINARY LITERALS
                    ...
                    while (ch == '0' || ch == '1') {
                        ...
                        reportScannerError("The Binary Literal is incorrect.");
                    } 
